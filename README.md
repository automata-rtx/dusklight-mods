...
| [WIP] Terrain Shadow Removal | `terrain_shadow_removal.dusk` | Removes the *other* fake shadow — the one TP scrolls across the terrain as an animated texture overlay (the drifting dappled shade on forest floors). Off by default; can make a floor look wrong if that room's shadow shares the base ground material, so test per area. Waterfalls/water unaffected. Experimental |
