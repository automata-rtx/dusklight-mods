...
| [WIP] Terrain Shadow Removal | `terrain_shadow_removal.dusk` | Removes the *other* fake shadow — the drifting dappled shade TP bakes into the terrain material as a second texture stage (not the "moya" particles). Zeroes the overlay's TEV strength register on the cloud-shadow terrain materials, so the shadow is cancelled but the base ground stays (no holes). Per-material-code toggles + live logger. Off by default; the big rolling Hyrule-Field cloud shadows (moya) are untouched. Experimental |
