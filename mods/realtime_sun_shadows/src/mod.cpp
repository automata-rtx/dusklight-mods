...
// volume scaled to match, an 8192 map option, and the contact-shadow parameters exposed.
//
...
void composite_map_pass(int64_t debugMode) {
    ...
    uniforms.contact_enabled = get_bool_option(g_cvarContactShadows, true) ? 1.0f : 0.0f;
    uniforms.contact_thickness =
        static_cast<float>(std::clamp<int64_t>(get_int_option(g_cvarContactThickness, 25), 5, 200));
    uniforms.contact_length =
        static_cast<float>(std::clamp<int64_t>(get_int_option(g_cvarContactLength, 60), 10, 500));
...
ModResult build_controls_tab(
    ...
    add_toggle(left, "Contact Shadows", g_cvarContactShadows,
        "Adds a screen-space raymarch for small-scale contact darkening the map misses - also "
        "re-grounds objects when bias settings push the mapped shadow away from their base.");
    add_number(left, "Contact Thickness", g_cvarContactThickness, 5, 200, 5, nullptr,
        "How thick a surface is assumed to be for the contact raymarch, in view units. Higher "
        "catches more contacts but can over-darken thin geometry.");
    add_number(left, "Contact Length", g_cvarContactLength, 10, 500, 10, nullptr,
        "How far the contact raymarch travels toward the light, in view units.");
...
MOD_EXPORT ModResult mod_initialize(ModError* error) {
    ...
    result = register_int_option("contactThickness", 25, g_cvarContactThickness, error);
    if (result != MOD_OK) {
        return result;
    }
    result = register_int_option("contactLength", 60, g_cvarContactLength, error);
    if (result != MOD_OK) {
        return result;
    }
