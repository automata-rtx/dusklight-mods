...
| :-- | :-- | :-- |
| `orbitEnabled` | on | Off = the positions are never touched, bit-exact vanilla (and the shadow mod falls back to the vanilla orbit). This is the only way to get the *exact* 59.036 vanilla tilt back, since the knob steps in whole degrees. **Not named `enabled`** — a var by that name formats to `mod.<escaped id>.enabled`, the key the **loader** reserves for the mod manager's own per-mod checkbox, so `register_var` rejects it and the mod fails to load entirely; see `docs/mod-api-notes.md` "Config/UI". The two differ in kind: the manager's checkbox unloads the mod, this one keeps it loaded and exporting its service while reporting the vanilla orbit. |
